...
def update_file_ep(fid: int, payload: FileUpdate, db: Session = Depends(get_db), _: models.User = Depends(get_current_admin_user)):
    ...
    db.refresh(f)
    result = FileResponse.model_validate(f)
    result.url = get_file_url(f.file_path)
    return result
